...
def readTag(page):
    ...
    while(readingLoop):
        try:
            ...
            if(dataCurr is not None):
                print dataCurr
                f=open("/tmp/nfc/nfc.id","w+")
                f.write(dataCurr)
                f.close()
                #break            

            LOADKEY = [0xFF, 0x00, 0x00, 0x00, 0x0F, 0xD4, 0x40, 0x01, 0x61, int(page), 255,255,255,255,255,255, resp[0][0], resp[0][1], resp[0][2], resp[0][3]]
            response = connection.transmit(LOADKEY)
            if response[1] == 144:
                #print("Key A loaded successfully")
                #auth block
                COMMAND = [0xFF, 0x00, 0x00, 0x00, 0x05, 0xD4, 0x40, 0x01, 0x30, int(page)]
                response = connection.transmit(COMMAND)
                print(response)
                dataCurr = stringParser(response)
                truncated_list = slice(3, len(response[0]))
                _response = response[0][truncated_list]
                #print (response)
                data_memory = ''.join(chr(i) for i in _response if i!=0)
                if (len(data_memory) > 0):
                    print("*"+data_memory+"*")
                    print(len(data_memory))
                    f=open("/tmp/nfc/"+page,"w+")
                    f.write(data_memory)
                    f.close()
